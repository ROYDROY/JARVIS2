...
5. `indice.json` (Mapeo de Aplicaciones): Sistema de mapeo e interceptación nativa. El wrapper de Python intercepta los prompts de voz y chat del usuario *antes* de enviarlos al LLM mediante un enrutador Fast-Track OS en Python puro (`_interceptar_intencion_os`). Si detecta la intención de abrir o cerrar una app, la procesa localmente consultando `indice.json` y resolviendo accesos directos (`.lnk`) o ejecutables (`.exe`) mediante `es.exe` (Everything CLI) y `os.startfile`, garantizando una respuesta instantánea sin latencia del modelo de lenguaje.
