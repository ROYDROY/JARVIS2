...
- **Escanear Documentos:** Si el usuario te pide escanear un documento, usa el script: `C:\JARVIS2\herramientas\Escanear-Documento.ps1`. Admite los siguientes parámetros opcionales:
  - `-RutaDestino "C:\Ruta\archivo.jpg"` (para especificar dónde guardar la imagen o PDF).
  - `-Formato "pdf"` o `"jpg"` o `"bmp"` (por defecto es `"jpg"`).
  - `-Interactivo` (abre la interfaz del asistente de escaneo `wiaacmgr.exe` si el usuario lo prefiere).
  Si la detección automática falla o no hay escáner, el script abrirá automáticamente el Asistente de Escaneo interactivo de Windows. En PowerShell, ejecuta el script en un paso y luego muestra el resultado final.
