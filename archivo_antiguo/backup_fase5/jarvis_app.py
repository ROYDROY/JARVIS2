...
class JarvisApp(ctk.CTk):
    ...
    def generar_respuesta_llm(self, prompt):
        try:
            ...
            import requests
            def resumir_intencion(text):
                try:
                    url = "http://localhost:11434/api/generate"
                    payload = {"model": "llama3.1:8b", "prompt": f"Resume ultra corto (máx 10 palabras) lo que vas a hacer al recibir: '{text}'. Empieza con 'Voy a...'.", "stream": False}
                    return requests.post(url, json=payload, timeout=5).json().get("response", "Voy a procesar tu petición.").strip()
                except:
                    return "Voy a procesar tu orden."

            resumen = resumir_intencion(prompt)
